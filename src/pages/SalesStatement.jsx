...
export default function SalesStatement() {
    ...
    }, [data, currentUser, statusFilter, paymentMethodFilter, repFilters, leaderFilters, dealIdFilter]);

    // Options for Filters
    const { statusOptions, paymentMethodOptions, repOptions, leaderOptions, contestantOptions } = useMemo(() => {
        if (!data) return { statusOptions: [], paymentMethodOptions: [], repOptions: [], leaderOptions: [], contestantOptions: [] };
        ...
        return {
            ...
            repOptions: reps,
            leaderOptions: Array.from(new Set(data.map(s => s.direct_leader).filter(Boolean))).sort(),
            contestantOptions: Array.from(new Set([...reps, ...quals])).sort()
        };
    }, [data]);

    // Metrics
    const totalRevenue = useMemo(() => filteredSales.reduce((sum, sale) => sum + (Number(sale.value) || 0), 0), [filteredSales]);
    const totalRevenueWithMultipliers = useMemo(() => filteredSales.reduce((sum, sale) => {
    ...
    const pendingCount = filteredSales.filter(s => s.commission_status === 'Pending').length;
    ...
    const handleAction = async (id, action) => {
        try {
            if (action === 'validate') {
                ...
                setSelectedDispute(filteredSales.find(s => s.id === id));
            }
        } catch (err) {
    ...
    return (
        <div className="font-sans text-[#0B1B2B]">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-serif font-bold text-[#0B1B2B]">Extrato de Vendas</h1>
                <button
            ...
            <div className="mb-8 grid grid-cols-1 md:grid-cols-4 gap-4 items-stretch">
                ...
                <MetricCard label="Receita Gerada" value={totalRevenue} isCurrency />
                <MetricCard label="Receita com Multiplicadores" value={totalRevenueWithMultipliers} isCurrency />
                <button
            ...
            <div className="bg-gray-50/50 p-4 rounded-xl border border-gray-100 mb-6 grid grid-cols-1 md:grid-cols-6 gap-4 items-end transition-all duration-300">
                ...
                <div className="flex-1 min-w-[200px]">
                    <label className="block text-[10px] text-gray-400 uppercase tracking-widest font-bold mb-1.5 ml-1">Forma de Pagamento</label>
                    <select
                    ...
                    <table className="w-full text-left border-collapse whitespace-nowrap">
                        <thead className="bg-[#F8F9FA] text-[#0B1B2B] border-b border-gray-200">
                            <tr>
                                ...
                                <th className="p-4 font-bold text-xs uppercase tracking-wider">Líder Direto</th>
                                <th className="p-4 font-bold text-xs uppercase tracking-wider">Data de fechamento</th>
                                <th className="p-4 font-bold text-xs uppercase tracking-wider">Nome do negócio</th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {filteredSales.map(sale => (
                                <tr key={sale.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="p-4 text-xs text-gray-500">{sale.direct_leader}</td>
